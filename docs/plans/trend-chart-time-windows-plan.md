...
1. **Project detail page** (`[owner]/[repo]/+page.svelte`): a category with very little history
   ...
   picks (observed spanning _years_ into the future on a single-point series). There's currently
...
- The list page's overlay sparkline always uses a fixed **30d** window, with lines aligned to a
  ...
  reporting cadence.
- In both cases, when there's no real data at the window's start, the closest point _before_ the
...
_what data the backend returns_ — inject a synthetic anchor point at the window boundary, and the
...
- **Anchor point**: the most recent row with `ran_at <= windowStart` (checking raw `coverage_runs`
  ...
  `src/db/rollup.ts:3`). Its value is re-emitted as a synthetic point _at_ `windowStart` (i.e.
...
left as above (relative to the shared window), _and_ if a category's own latest point is earlier
...
- **Regression guard for the `recorded_at` trap**: for a `15m`/`1h`/`12h` window, assert the
  anchor point and the latest point have _distinct_ timestamps (not just distinct values) — this
